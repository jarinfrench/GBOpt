...
interface separation, atom insertion/removal, and single-mode soft-phonon
displacement). Registration in ``default_registry`` is still always explicit --
importing this module registers no built-in operation under a name; it only makes the
operation classes importable. ``GBManipulator``'s own legacy methods
(``translate_right_grain``, ``cycle_grain_terminations``, ``apply_interface_separation``,
``insert_atoms``, ``remove_atoms``, ``displace_along_soft_modes``, and their
``make_*_candidate`` counterparts) share these operations' pure computational core and
remain the supported entry points for that scripted usage.
"""
