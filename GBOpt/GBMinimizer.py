...
@dataclass(frozen=True, slots=True)
class _GASelectionPolicy:
    """Validated candidate-selection policy for the genetic algorithm.

    Percentages are converted after failed evaluations have been removed from the
    candidate pool. A positive percentage therefore selects at least one candidate
    from any nonempty valid pool while preserving floor-based percentage conversion
    for all larger selections.

    :param keep_top_pct: Percentage of lowest-energy candidates retained unchanged.
        Zero explicitly disables elitist carryover.
    :param intermediate_pct: Percentage of lowest-energy candidates eligible to breed.
        Must be positive because a nonempty parent pool is required to create
        offspring.
    """
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            self._normalize_percentage(
                "intermediate_pct",
                self.intermediate_pct,
                minimum=1,
            ),
        )

    @staticmethod
    def _normalize_percentage(name: str, value: object, *, minimum: int) -> int:
        """Return one validated integer selection percentage.

        :param name: Configuration-field name used in error messages.
        :param value: Percentage value supplied by the caller.
        :param minimum: Keyword argument, required. Smallest accepted percentage.
        :return: Normalized Python integer percentage.
        :raises GBMinimizerTypeError: If ``value`` is not a non-Boolean integer.
        :raises GBMinimizerValueError: If ``value`` lies outside the accepted range.
        """
        if isinstance(value, (bool, np.bool_)) or not isinstance(value, Integral):
            raise GBMinimizerTypeError(f"{name} must be a non-Boolean integer")
        value = int(value)
        if not minimum <= value <= 100:
            raise GBMinimizerValueError(
                f"{name} must be between {minimum} and 100"
            )
        return value

    @staticmethod
    def _selection_count(candidate_count: int, percentage: int) -> int:
        """Convert a validated percentage to a candidate count.

        :param candidate_count: Number of valid evaluated candidates.
        :param percentage: Validated selection percentage.
        :return: Number of candidates selected by the policy.
        """
        if candidate_count == 0 or percentage == 0:
            return 0
        return max(1, (candidate_count * percentage) // 100)

    def select(self, energies: list[float]) -> tuple[list[int], list[int]]:
        """Select elite and breeding-pool indices by ascending objective.

        :param energies: Objectives for the valid candidates under consideration.
        :return: Indices of carryover elites and breeding-pool candidates.
        """
        indices = sorted(range(len(energies)), key=energies.__getitem__)
        elite_count = self._selection_count(len(energies), self.keep_top_pct)
        parent_count = self._selection_count(
            len(energies),
            self.intermediate_pct,
        )
        return indices[:elite_count], indices[:parent_count]
...
class GeneticAlgorithmMinimizer:
    ...
    def __init__(
        ...
        *,
        ...
        :param population_size: Keyword argument, optional, defaults to 20. Number of
            candidates per generation.
        :param generations: Keyword argument, optional, defaults to 50. Number of
            generations to iterate.
        :param keep_top_pct: Keyword argument, optional, defaults to 10. Percentage of
            lowest-energy structures carried over unchanged. Zero disables carryover;
            any positive value retains at least one valid candidate.
        :param intermediate_pct: Keyword argument, optional, defaults to 60. Percentage
            of lowest-energy structures eligible for crossover/mutation selection. Must
            be between 1 and 100; any valid value selects at least one candidate from a
            nonempty valid pool.
        :param slice_and_merge_pct: Keyword argument, optional, defaults to 50.0.
            Percentage of non-carryover offspring generated by slice-and-merge
            crossover. The remaining offspring are generated by mutation.
        :param reuse_carryover_evaluations: Keyword argument, optional, defaults to
            ``False``. Reuse the validated energy and relaxed artifact of unchanged
            successful carryover candidates instead of invoking the evaluator again.
        ...
        :raises GBMinimizerTypeError: If a GA selection percentage is not a non-Boolean
            integer.
        :raises GBMinimizerValueError: If ``keep_top_pct`` is outside 0--100 or
            ``intermediate_pct`` is outside 1--100.
        """
        selection_policy = _GASelectionPolicy(
    ...
    @property
    def keep_top_pct(self) -> int:
        """Return the validated elitist-carryover percentage."""
        return self._selection_policy.keep_top_pct

    @property
    def intermediate_pct(self) -> int:
        """Return the validated breeding-pool percentage."""
        return self._selection_policy.intermediate_pct
    ...
    def _make_next_owned_generation(
        ...
        :raises ValueError: If records or the required parent pool are empty, or if
            ``offspring_count`` is invalid.
        """
        if not records:
        ...
        if not intermediate_indices:
            raise ValueError(
                "intermediate_indices must contain at least one eligible parent"
            )
    ...
    def _make_next_generation(
        ...
        :param offspring_count: Number of unfilled population slots.
        :return: Aligned manipulators, atom arrays, and lineages.
        :raises ValueError: If parent files or the required parent pool are empty, or if
            ``offspring_count`` is invalid.
        """
        if not files:
            raise ValueError(
        ...
        **not** deleted on normal completion — it can be used to continue the run later
        ...
        for gen in range(_start_gen, self.generations):
            ...
            if not valid_old_idxs:
                ...
                lowest_valid_idxs, inter_valid_idxs = self._selection_policy.select(
                    valid_energies
                )
        ...
        *,
        ...
        for gen in range(_start_gen, self.generations):
            ...
            if not valid_records:
                ...
                lowest_indices, intermediate_indices = self._selection_policy.select(
                    valid_energies
                )
                next_manipulators = []
                next_structures = []
