...
class TestGBManipulator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.GB = GBMaker(a0=1.0, structure='fcc', gb_thickness=10.0, misorientation=[
    ...
    def test_init_with_one_gbmaker_parent(self):
        manipulator = GBManipulator(self.GB)
    ...
    def test_init_with_mixed_input(self):
        ...
        gb_thickness = 10
        manipulator = GBManipulator(
            self.GB, self.file1, unit_cell=unit_cell, gb_thickness=gb_thickness)
        self.assertIsNotNone(manipulator.parents[0])
        self.assertIsNotNone(manipulator.parents[1])

        manipulator2 = GBManipulator(
            self.file1, self.GB, unit_cell=unit_cell, gb_thickness=gb_thickness)
    ...
    def test_grain_translation(self):
        ...
        self.assertTrue(not np.allclose(self.GB.gb, new_system))
    ...
    def test_slice_and_merge(self):
        ...
        self.assertFalse(np.allclose(self.GB.gb, new_system))
    ...
    def test_insert_atoms(self):
        ...
        self.assertGreater(len(new_system1), len(self.GB.gb))
        new_system2 = manipulator.insert_atoms(0.10, method='grid')
        self.assertGreater(len(new_system2), len(self.GB.gb))

    def test_insert_atoms_fraction_error(self):
        manipulator = GBManipulator(self.GB, seed=self.seed)
        with self.assertRaises(GBManipulatorValueError):
            _ = manipulator.insert_atoms(0.50, method='delaunay')

    def test_insert_atoms_2_parent_warning(self):
        manipulator = GBManipulator(self.GB, self.GB, seed=self.seed)
        with self.assertWarns(UserWarning):
            _ = manipulator.insert_atoms(0.10, method='delaunay')

    def test_insert_atoms_calculated_fraction_warning(self):
        manipulator = GBManipulator(self.GB, seed=self.seed)
        with self.assertWarns(UserWarning):
            _ = manipulator.insert_atoms(0.0000001, method='delaunay')
    ...
    def test_parents_setter(self):
        manipulator = GBManipulator(self.GB)
        self.assertIsNone(manipulator.parents[1])
        manipulator.parents[1] = Parent(self.GB)
        self.assertIsNotNone(manipulator.parents[1])
...
class TestParent(unittest.TestCase):
    ...
    def test_parent_init(self):
        ...
        self.assertEqual(len(parent1.left_grain) +
                         len(parent1.right_grain), len(parent1.whole_gb))
        ...
        self.assertEqual(len(parent2.left_grain) +
                         len(parent2.right_grain), len(parent2.whole_gb))
